...
def select_series_title_with_most_human_characters
  "select title from (select b.title, count(a.id) from characters a left join series b on a.series_id = b.id where species = 'human' group by b.series_name order by count(a.id) desc) d limit 1"
end
