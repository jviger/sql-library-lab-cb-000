...
def select_series_title_with_most_human_characters
  "select name from (select b.name, count(character_id) from characters a left join series b on a.series_id = b.id where species = 'human' group by b.series_name order by count(character_id) desc) d limit 1"
end
